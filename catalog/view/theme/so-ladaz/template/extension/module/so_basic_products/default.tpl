	...
	<div class="so-basic-product" id="<?php echo $tag_id ?>">
		<div class="item-wrap row cf products-list grid">
			<?php if(isset($products) && !empty($products)){
			 $j = 0; foreach($products as $product)  { $j++; ?>
					...
					<?php if($display_add_to_cart || $display_wishlist || $display_compare)
							...
							?>
								...
								<?php if ($display_add_to_cart) { ?>
									<button type="button" class="addToCart" data-toggle="tooltip" title="<?php echo $objlang->get('button_cart'); ?>"  onclick="cart.add('<?php echo $product['product_id']; ?>');"><i class="fa fa-shopping-cart"></i> </button>
								<?php } ?>
			...
			<?php 
				...
				echo '<div class="alert alert-danger"><i class="fa fa-exclamation-circle"></i>No Item Product</div>';
			}
			?>
		</div>
	</div>
